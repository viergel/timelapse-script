...
while true; do
  ...
  for monitor in $MONITORS; do
    grim -o "$monitor" "$BASE_PATH/$monitor/$timestamp.png"
  done
  ...
  if ((frame_count % 60 == 0)); then
    ...
    ffmpeg -y -i "$BASE_PATH/HDMI-A-1/temp-2.mp4" -i "$BASE_PATH/webcam/temp-2.mp4" -i "$BASE_PATH/eDP-1/temp-2.mp4" -filter_complex "\
    [0:v]scale=1920:1080[bg]; \
    [1:v]scale=587:330[small1]; \
    [2:v]scale=587:330[small2]; \
    [bg][small1]overlay=0:main_h-overlay_h[tmp]; \
